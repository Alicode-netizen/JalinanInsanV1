...
with tabs[0]:
    ...
    bio = st.session_state.user_data.get("bio", {}) if st.session_state.authenticated else {}
    st.markdown(f"**Name:** {bio.get('name', 'Not set')}")
    st.markdown(f"**Email:** {st.session_state.user_email if st.session_state.authenticated else 'Not logged in'}")
...
with tabs[1]:
    ...
    if not history_data:
        ...
        for i, item in enumerate(history_data):
            ...
            if st.session_state.authenticated and st.button(f"❌ Delete this {item['type']}", key=f"delete_{i}"):
...
with tabs[2]:
    ...
    with st.form("bio_form"):
        ...
        email = st.text_input("Email", value=st.session_state.user_email if st.session_state.authenticated else "")
        ...
        if st.form_submit_button("Save"):
            if st.session_state.authenticated:
                ...
                st.warning("Please log in to save biodata.")
...
with tabs[3]:
    ...
    if tool == "Genogram":
        ...
        if 'current_genogram' in st.session_state:
            if st.session_state.authenticated and st.button("💾 Save to History", key="save_genogram"):
                history_entry = {
                    "type": "Genogram",
                ...
                st.warning("Please log in to save to history.")
    ...
    # ---------- Ecomap ----------
        ...
        with col2:
            ...
            if st.session_state.authenticated and st.button("💾 Save to History", key="save_ecomap"):
    ...
    # ---------- Social Network ----------
        ...
        with col2:
            ...
            if st.session_state.authenticated and st.button("💾 Save to History", key="save_social"):
        ...
        with st.form("life_form", clear_on_submit=True):
            ...
            if submitted_life and time and event:
                if st.session_state.authenticated:
                    ...
                    st.warning("Please log in to save life events.")
        if st.session_state.authenticated and st.session_state.user_data.get("lifemap"):
            ...
            st.warning("Please log in to view and save your life roadmap.")
